    ...
    >
      ...
      <div className="space-y-4 mb-6">
        <div className="flex justify-between">
          ...
          <span className="font-medium">{cartTotal.toFixed(2)}XOF</span>
        </div>

        <div className="flex justify-between">
          <span className="text-gray-600">Livraison</span>
          <span className="font-medium">
            {selectedDelivery ? `${selectedDelivery.name} (${deliveryCost.toFixed(2)}XOF)` : 'Non sélectionnée'}
          </span>
        </div>

        <div className="border-t border-gray-200 pt-4 flex justify-between">
          <span className="font-bold">Total</span>
          <span className="font-bold text-xl text-primary-600">{grandTotal.toFixed(2)}XOF</span>
        </div>
      </div>
          ...
          <div className="mt-2 border border-gray-200 rounded-lg p-4">
            {deliveryOptions.map(option => (
              ...
              >
                ...
                <span className="font-medium">{option.price}XOF</span>
              </label>
            ))}
          </div>
        )}
